...
int Application::drawButtons(Mat &display)
{
    ...
    putText(display, "median", 
        Point(guiState.onButtonPlace.x + guiState.onButtonPlace.width / 2 - 55,
    ...
    putText(display, "gray", 
        Point(guiState.cvtColorButtonPlace.x + guiState.cvtColorButtonPlace.width / 2 - 40, guiState.cvtColorButtonPlace.y + guiState.cvtColorButtonPlace.height / 2 + 10), FONT_HERSHEY_SIMPLEX, 1.0, Scalar(0, 0, 0), 2);
    putText(display, "pixel", 
        Point(guiState.pixelButtonPlace.x + guiState.pixelButtonPlace.width / 2 - 40, guiState.pixelButtonPlace.y + guiState.pixelButtonPlace.height / 2 + 10), FONT_HERSHEY_SIMPLEX, 1.0, Scalar(0, 0, 0), 2);
    putText(display, "canny", 
        Point(guiState.cannyButtonPlace.x + guiState.cannyButtonPlace.width / 2 - 40, guiState.cannyButtonPlace.y + guiState.cannyButtonPlace.height / 2 + 10), FONT_HERSHEY_SIMPLEX, 1.0, Scalar(0, 0, 0), 2);
...
void onButtonsOnOffClick(int eventId, int x, int y, int flags, void *userData)
{
    ...
    if (onButtonClicked(elems->cvtColorButtonPlace, x, y))
    {
        elems->state = Application::OnFilter;
